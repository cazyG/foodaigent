...
//            .logger(DebugLogger())
    ...
    KoinApplication(
        ...
        content = {
            MaterialTheme {
                val navBackStack = rememberAppNavBackStack(initialDestination = AppRoute.Home)
                val currentTab = navBackStack.firstOrNull() as? AppRoute ?: AppRoute.Home
                val recipesRefreshKey = remember { androidx.compose.runtime.mutableStateOf(0) }
                val popBackStack = {
                    if (navBackStack.size > 1) {
                        navBackStack.removeAt(navBackStack.lastIndex)
                    } else if (currentTab != AppRoute.Home) {
                        navBackStack.clear()
                        navBackStack.add(AppRoute.Home)
                    }
                }
                val switchTab: (AppRoute) -> Unit = { route ->
                    ...
                    entryProvider = entryProvider {
                        entry<AppRoute.Home> {
                            IndexScreen(
                                ...
                                onAddPlan = { switchTab(AppRoute.Recipes) }
                            )
                        }
                        entry<AppRoute.Recipes> {
                            RecipesScreen(
                                activeTab = currentTab,
                                onTabClick = switchTab,
                                refreshTrigger = recipesRefreshKey.value,
                                onNavigateToManualInput = { navBackStack.add(AppRoute.ManualRecipeInput) }
                            )
                        }
                        entry<AppRoute.History> { HistoryScreen(activeTab = currentTab, onTabClick = switchTab) }
                        entry<AppRoute.Profile> { ProfileScreen(activeTab = currentTab, onTabClick = switchTab) }
                        entry<AppRoute.Plan> { PlanningScreen(activeTab = currentTab, onTabClick = switchTab) }
